...
# Variables to count workers and patients
total_workers = 0
unique_workers = set()
            ...
            for filename, encoding in worker_face_encodings:
                ...
                if face_distance[0] < face_recognition_threshold:
                    ...
                    if current_time - last_recognition_time > time_window:
                        ...
                        total_workers += 1
                        unique_workers.add(filename)
                    else:
                        category = "Worker Duplicate Patient"
                    break

            if not found_worker:
                for patient_encoding in unique_patients:
                    face_distance = face_recognition.face_distance([patient_encoding], captured_encoding)
                    if face_distance[0] < face_recognition_threshold:
                        found_patient = True
                        category = "Duplicate Patient"
                        duplicate_patients.add(captured_encoding)
                        break

                if not found_patient:
                    category = "Patient"
                    total_patients += 1
                    unique_patients.add(captured_encoding)
...
# Print the total worker and patient counts, as well as unique workers and duplicated patients
print("Total Workers:", total_workers)
print("Unique Workers:", unique_workers)

print("Total Patients:", total_patients)
print("Unique Patients:", len(unique_patients))
print("Duplicated Patients:", len(duplicate_patients))
